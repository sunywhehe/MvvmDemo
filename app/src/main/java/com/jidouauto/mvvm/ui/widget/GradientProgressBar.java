...
public class GradientProgressBar extends View {
    ...
    public GradientProgressBar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.GradientProgressbar);
        progressStartColor = ta.getColor(R.styleable.GradientProgressbar_progressStartColor, Color.YELLOW);
        progressEndColor = ta.getColor(R.styleable.GradientProgressbar_progressEndColor, progressStartColor);
        bgStartColor = ta.getColor(R.styleable.GradientProgressbar_bgStartColor, Color.LTGRAY);
        bgMidColor = ta.getColor(R.styleable.GradientProgressbar_bgMidColor, bgStartColor);
        bgEndColor = ta.getColor(R.styleable.GradientProgressbar_bgEndColor, bgStartColor);
        progress = ta.getInt(R.styleable.GradientProgressbar_progress, 0);
        progressWidth = ta.getDimension(R.styleable.GradientProgressbar_progressWidth, 8f);
        startAngle = ta.getInt(R.styleable.GradientProgressbar_startAngle, 0);
        sweepAngle = ta.getInt(R.styleable.GradientProgressbar_sweepAngle, 360);
        showAnim = ta.getBoolean(R.styleable.GradientProgressbar_showAnim, true);
        ta.recycle();
